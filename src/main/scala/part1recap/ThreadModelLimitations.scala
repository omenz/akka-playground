...
//  println(account.getAmount)
  ...
  // synchronization! Locks to the rescue - deadlocks, livelocks
  ...
  Thread.sleep(1000)
  delegateBackgroundThread(() => println("This should run in the background"))

  /**
    * DR #3: tracing and dealing with error is a multithreaded env is PITA
    */
  // 1M numbers in between 10 threads
  import scala.concurrent.ExecutionContext.Implicits.global

  val futures = (1 to 9)
    .map(i => 100000 * i until 100000 * (i + 1)) // 0 - 999999, 100000 - 199999, 200000 - 299999 etc
    .map(range => Future {
  ...
  sumFuture.onComplete(println) //good luck finding the source of exception
