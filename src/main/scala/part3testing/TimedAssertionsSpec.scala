...
class TimedAssertionsSpec extends TestKit(
  ...
  "A worker actor" should {
    ...
    "reply with the meaning of life in a timely manner" in {
      within(500 millis, 1 second) { // time box, between 500ms and 1 seconds
        workerActor ! "work"
    ...
    "reply to a test probe in a timely manner" in {
      within(1 second) { //this within block will not affect probe timeout
        val probe = TestProbe()
        probe.send(workerActor, "work")
        probe.expectMsg(WorkResult(42)) // timeout of 0.3 seconds in the application.conf
      }
    }
  }
}
