...
class SupervisionSpec extends TestKit(ActorSystem("SupervisionSpec"))
  ...
  "A supervisor" should {
    ...
    "restart its child in case of an empty sentence" in {
      ...
      child ! ""
      child ! Report
      expectMsg(0) // state is cleared after restart
    }
  ...
  "An all-for-one supervisor" should {
    "apply the all-for-one strategy" in {
      ...
      Thread.sleep(100)
      secondChild ! Report
      expectMsg(0) //other actor state is cleared because the first one has failed and all for one strategy was applied
    }
  }
}
...
object SupervisionSpec {
  ...
  class AllForOneSupervisor extends Supervisor {
    // applied for all child actors even if only one has failed
